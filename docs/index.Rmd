...
Oscars.Original <- Oscars

##################################
# Formulating dataset for
# Principal Component Analysis
##################################
Oscars <- Oscars[,c("Tomatometer_Critic",
                    "Tomatometer_Audience",
                    "IMDB_Critic",
                    "IMDB_Audience",
                    "Nominations_Total",
                    "Nomination_SuccessRate")]
row.names(Oscars) <- Oscars.Original$Film
colnames(Oscars) <- c("TCTC",
                      "TAUD",
                      "ICTC",
                      "IAUD",
                      "ONOM",
...
##################################
Oscars.FA_CenteredScaled <- preProcess(Oscars.FA, method = c("center","scale"))
Oscars.FA <- predict(Oscars.FA_CenteredScaled, Oscars.FA)
...
# Applying a center and scale data transformation
##################################
Oscars.FA_CenteredScaledRanged <- preProcess(Oscars.FA, method = c("range"), rangeBounds = c(0, 1))
Oscars.FA <- predict(Oscars.FA_CenteredScaledRanged, Oscars.FA)
...
##################################
(DPA_Correlation <- cor(Oscars.FA,
                        method = "pearson",
                        use="pairwise.complete.obs"))

DPA_CorrelationTest <- cor.mtest(Oscars.FA,
                       method = "pearson",
                       conf.level = 0.95)

corrplot(cor(Oscars.FA,
             method = "pearson",
             use="pairwise.complete.obs"), 
             method = "circle",
             type = "upper", 
...
qgraph(cor(Oscars.FA),
       cut=0.30,
       details=TRUE,
       posCol="#2F75B5",
       negCol="#FF5050",
       labels=names(Oscars.FA))
...
##################################
(DPA_CorrelationMatrixDeterminant <- det(cor(Oscars.FA)))

```

</details>

###  1.4.2 Correlation Matrix Assessment - Bartlett’s Test of Sphericity
|
|
|
|
...
##################################
(DPA_BartlettTest <- cortest.bartlett(Oscars.FA,
                                      n=nrow(Oscars.FA)))
...
##################################
(DPA_KMOFactorAdequacy <- KMO(Oscars.FA))
(DPA_KMOFactorAdequacy <- KMO(Oscars.FA[,!names(Oscars.FA) %in% c("ONOM")]))
(DPA_KMOFactorAdequacy <- KMO(Oscars.FA[,!names(Oscars.FA) %in% c("OWIN")]))
(DPA_KMOFactorAdequacy <- KMO(Oscars.FA[,!names(Oscars.FA) %in% c("ONOM","OWIN")]))
...
splom(~EDA_Oscars,
      ...
      main = "Exploratory Analysis Between Quantitative Descriptors",
...
##################################
set.seed(88888888)
(FA_PA_V_ParallelAnalysis <- fa.parallel(Oscars.FA,
                                        fm="pa",
                                        fa="fa",
                                        n.iter=500,
                                        plot=FALSE))
...
# of the same size as the original
##################################
(FA_PA_V_MinimumAveragePartialAnalysis <- VSS(Oscars.FA,
                                              plot=FALSE))
...
##################################
FA_PA_V <- fa(Oscars.FA,
              nfactors = 2,
              fm="pa",
              rotate = "varimax",
              residuals=TRUE,
              SMC=TRUE,
              n.obs=nrow(Oscars.FA))
...
# for each factor
##################################
alpha(Oscars.FA[,c("ICTC","TCTC","ONOM")])
alpha(Oscars.FA[,c("IAUD","TAUD","OWIN")])
...
##################################
FA_PA_V_FactorLoading <- factload(Oscars.FA,
                                  cormeth = "pearson",
                                  method = "prax",
                                  nfac = 2,
                                  rotation = "varimax")
...
# FA_ML_V_FactorLoading <- factload(Oscars.FA, 
#                                   cormeth = "pearson",
#                                   method = "mle", 
#                                   nfac = 2, 
#                                   rotation = "varimax")
